...
void Turn_around(int n,int BT[],string p[],int Arrival_time[])
{
    int Finish_time[10],TA[10],WT[10];
    float cnt=0,count=0;

    int sum=Arrival_time[0];
    for (int i = 0; i<n; i++)
    {
        sum=sum+BT[i];
        Finish_time[i]=sum;
   ...
   /* for (int i = 0; i<n; i++)
    {
        if(i==0)
        {   Finish_time[i]=++BT[i];
            TA[i]=Finish_time[i]-Arrival_time[i];
        }
        else
        {*/
...
int main()
{
    ...
    Turn_around(n,BT,p,Arrival_time);
    return 0;
}
